...
if (family == "T")
{
 ...
 betas      <- as.matrix(Abetas[2:(p+1)])   ### parametros da regressao dimensao "p"
 ...
 { #Start the While
  ...
  #means of normal mixtures. Computational Statistics, 12(1), 1?18.
  teta  <- c(Abetas, medj, sigma2, pii,nu)
  }
...
if (family == "Normal")
{
 ...
 if (criteria == TRUE)
 {
  if((family == "Normal")) d <- p+1+ g*2 + (g-1) #Abeta + varphi, Sigma + pi
    ...
    icl     <- -2*icl + log(n)*d
    obj.out <- list(Abetas = Abetas, medj=medj, sigma2 = sigma2, nu=nu, pii = pii,  aic = aic, bic = bic, edc = edc, icl= icl, loglik=lk, iter = count, n = length(y), group = cl)#, im.sdev=NULL)
  }
 else obj.out <- list(Abetas = Abetas, medj=medj , sigma2 = sigma2, nu=nu, pii = pii, iter = count, n = length(y), group = apply(tal, 1, which.max))#, im.sdev=NULL)
